...
TArray<UEdGraphNode*> FEdGraphFormatter::GetNodeTree(UEdGraphNode* InitialNode) const
{
	...
	{
		return ShouldFormatNode(Link.GetToNodeUnsafe()) && FBAUtils::IsNodeImpure(Link.GetToNodeUnsafe()) && GraphHandlerCapture->FilterDelegatePin(Link, FormatterParametersCapture.NodesToFormat.GetCachedNodes());
	};
...
void FEdGraphFormatter::PostFormatting()
{
	...
	ConnectionValidator.CheckChanged(FBAUtils::GetNodeTree(GetRootNode()).Array());
